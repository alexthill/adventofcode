...
// Registers only need to be u32 for this day but u64 for day21.
// It would run this day faster with u32 but it is already fast
// and don't want to do generic stuff.
pub fn parse_instructions(input: &str) -> (usize, Vec<(u8, [u64; 3])>) {
    ...
    let instructions = lines.map(|line| {
        ...
        let args = [next_parse!(parts, u64), next_parse!(parts, u64), next_parse!(parts, u64)];
        (code, args)
    }).collect::<Vec<_>>();
    (ip, instructions)
}

pub fn exec_op(code: u8, args: [u64; 3], mut registers: [u64; 6]) -> [u64; 6] {
...
impl Day for Day19 {
    ...
    fn solve(input: &str) -> [Solution; 2] {
        ...
        fn get_num(instructions: &[(u8, [u64; 3])], ip: usize, mut registers: [u64; 6]) -> u64 {
            let mut old_max = 0;
        ...
        fn sum_divisors(num: u64) -> u64 {
            let to = (num as f32).sqrt().ceil() as u64;
            (1..to).filter_map(|n| {
                if num % n == 0 { Some(n + num / n) } else { None }
            }).sum::<u64>() + if to * to == num { to } else { 0 }
        }
